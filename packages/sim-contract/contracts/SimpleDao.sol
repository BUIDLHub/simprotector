...
contract SimpleDao {
    ...
    modifier timeLineCheck(uint32 _blockNumber, uint16 _commitBalkline, uint16 _commitDeadline) {
        ...
        if (block.number >= _blockNumber - _commitBalkline) revert();
        _;
    }
    ...
    function newCampaign(
        ...
        timeLineCheck(_blockNumber, _commitBalkline, _commitDeadline)
        ...
        emit LogCampaignAdded(_campaignID, msg.sender, _blockNumber, _deposit, _commitBalkline, _commitDeadline, msg.value);
    }
    ...
    modifier checkCommitPhase(uint256 _blockNumber, uint16 _commitBalkline, uint16 _commitDeadline) {
        if (block.number < _blockNumber - _commitBalkline) revert();
        if (block.number > _blockNumber - _commitDeadline) revert();
        _;
    }
    ...
    function commitmentCampaign(
        ...
        checkCommitPhase(c.blockNumber, c.commitBalkline, c.commitDeadline)
    ...
    modifier checkRevealPhase(uint256 _blockNumber, uint16 _commitDeadline) {
        if (block.number <= _blockNumber - _commitDeadline) revert();
        if (block.number >= _blockNumber) revert();
        _;
    }
    ...
    ) checkRevealPhase(c.blockNumber, c.commitDeadline)
    ...
    modifier bountyPhase(uint256 _blockNumber){if (block.number < _blockNumber) revert(); _;}
    ...
    function returnRandom(Campaign storage c) internal bountyPhase(c.blockNumber) returns (uint256) {
